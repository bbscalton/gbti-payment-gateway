/**
 * Hosted checkout HTML — card fields post ONLY to the mock processor endpoint.
 * Merchant backend never receives PAN / expiry / CVV.
 */
...
function renderCheckoutPage({ order, publicBaseUrl }) {
  const amountGyd = (order.amountCents / 100).toLocaleString("en-GY", {
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .panel {
      background: var(--gbti-cream);
      border-radius: 4px;
      padding: 22px 20px 24px;
      box-shadow: 0 12px 40px rgba(0,0,0,0.25);
      border-top: 3px solid var(--gbti-gold);
    }
    .summary {
    ...
    input:focus {
      outline: 2px solid var(--gbti-gold);
      border-color: var(--gbti-gold);
    }
  ...
  <script>
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        err.textContent = 'Network error. Is the sandbox backend running?';
        err.style.display = 'block';
      } finally {
        btn.disabled = false;
      }
...
function renderResultPage({ order, success }) {
  const amountGyd = (order.amountCents / 100).toLocaleString("en-GY", {
    minimumFractionDigits: 2,
    maximumFractionDigits: 2,
  });
...
module.exports = { renderCheckoutPage, renderResultPage };
